		...
		opts = {
			...
			servers = {
				...
				yamlls = {
					...
					--     },
					--     schemas = {
					--       ["https://raw.githubusercontent.com/yannh/kubernetes-json-schema/master/v1.25.9/all.json"] = "/helm/*.yaml",
		...
		config = function(_, opts)
			...
			Util.on_attach(function(client, buffer)
				...
				if client.config.name == "yamlls" and vim.bo.filetype == "helm" then
					vim.lsp.stop_client(buffer, client.id)
				end
			end)
			...
			return {
				...
				sources = {
					...
					nls.builtins.diagnostics.yamllint.with({
						args = { "--format", "parsable", "-", "-d", "{rules: {line-length: disable}}" },
					}),
